...
const createapplink = async (req, res) => {

    const { link } = req.body;

    try {
        const result = await pool.query(
            'INSERT INTO Applink (link) VALUES ($1) RETURNING *',
            [link]
        );
...
const getapplinkbyID = async (req, res) => {
    const linkID = req.params.id;
    try {
        const result = await pool.query(
            'SELECT * FROM Applink WHERE id=$1',
            [linkID] // Pass the linkID as a parameter
        );
...
const deleteapplink = async (req, res) => {
    const linkID = req.params.id;
    try {
        const result = await pool.query('DELETE FROM Applink WHERE id = $1', [linkID]);
...
const updateApplinkByID = async (req, res) => {
    const linkID = req.params.id;
    const { link } = req.body;

    try {
        const result = await pool.query(
            'UPDATE Applink SET link = $1, updated_at = NOW() WHERE id = $2 RETURNING *',
            [link, linkID]
        );
...
module.exports = { createapplink, getapplink, getapplinkbyID, deleteapplink, updateApplinkByID };